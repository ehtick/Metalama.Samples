...
In this article, we'll create the simplest possible aspect to automatically implement the equality comparison pattern. We'll call it `[ImplementEquatable]` because it implements the `IEquatable` interface.

Of course, just implementing the `IEquatable` interface isn't enough for a complete equality comparison pattern. For a type `T`, a full equality comparison pattern involves the following operations:
...
An aspect should be fully deterministic, generating identical outputs from identical inputs in any situation. Therefore, we must order the collection of fields and automatic properties. For simplicity, we'll order by member name, but we'll revisit this in Article 4.
...
Our first piece of advice is to add the `IEquatable<T>` interface to the target type, where the generic parameter `T` is replaced with the target type itself.
...
- A run-time parameter `other` that's part of the `IEquatable<T>.Equals(T)` method signature.
...
Note that we're relying on the default `EqualityComparer` for each field type. In Article 4, we'll explore how to customize this logic.
...
Next, let's override the default `Equals` method, so facilities that don't support the `IEquatable` interface use the correct equality implementation.
