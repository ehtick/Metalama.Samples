...
# Equality comparison example, step 2: Supporting type inheritance
...
For `GetHashCode`, we also need to call the base method to integrate the base members into the hash code. However, we should be careful _not_ to call the root `object.GetHashCode()` because it returns a hash of the object's address.
...
Remember that we decided, in the previous article, to split the `BuildAspect` method in two parts: first information gathering, then adding advise. We need to enhance the first part and identify artifacts from the base classes.
...
Note the use of `With(InvokerOptions.Base)`. By default, invoking a method (using `IMethod.Invoke`) results in calling the final override (i.e., generating `this.Equals`) of this method, even if the method is in a base type. You must use the `InvokerOptions.Base` option so that Metalama generates `base.Equals`.
...
1. In `BuildAction`, pass `baseGetHashCodeMethod` to `IntroduceMethod`.
